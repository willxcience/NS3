...
void handler(NetDeviceContainer enbLteDevs)
{
    cout << Simulator::Now().As(Time::S) << endl;

    //change enb0 tx power to 0

    for (short i = 0; i < enbLteDevs.GetN(); i++)
    {
        ...
        cout << i << endl;
        enbPhy->SetAttribute("TxPower", DoubleValue(0.0));
    }
}
...
int main(int argc, char *argv[])
{
    ...
    Simulator::Schedule(Seconds(0.1), &handler, homeEnbDevs);
    Simulator::Schedule(Seconds(0.1), &handler, macroEnbDevs);
